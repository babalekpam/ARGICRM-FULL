...
export default function TeamPage() {
  ...
  const sendInvite = useMutation({
    ...
    onError:(e:any)=>setInviteError(e.message||t("team_invite_failed","Failed to invite user"))
  });
  ...
  return (
    <Layout title={t("team_title")} subtitle={`${users.length} ${t("team_members")}`} actions={
      isAdmin&&<button data-testid="button-invite-member" onClick={()=>{ setShowInvite(true); setInviteResult(null); setInvite({email:"",firstName:"",lastName:"",role:"user"}); }} style={{display:"flex",alignItems:"center",gap:6,background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"7px 14px",fontSize:13,fontWeight:600,cursor:"pointer"}}><Plus size={14}/>{t("invite_member")}</button>
    }>

      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        {[
          {l:t("team_stat_total","Total Members"),v:users.length,I:Users,c:"#6366f1"},
          {l:t("active"),v:active,I:UserCheck,c:"#22c55e"},
          {l:t("inactive"),v:users.length-active,I:UserX,c:"#ef4444"},
          {l:t("team_stat_admins","Admins"),v:admins,I:Shield,c:"#f59e0b"}
        ].map(s=>(
      ...
      <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:10,overflow:"hidden"}}>
        {isLoading?<div style={{padding:48,textAlign:"center",color:"var(--text-muted)"}}>{t("team_loading","Loading team…")}</div>:users.length===0?(
          <div style={{padding:64,textAlign:"center"}}><Users size={36} style={{color:"var(--text-muted)",marginBottom:12,opacity:0.4}}/><div style={{fontSize:15,fontWeight:600,marginBottom:6}}>{t("no_members")}</div><div style={{fontSize:13,color:"var(--text-muted)",marginBottom:16}}>{t("team_empty_desc","Invite your first team member to collaborate")}</div>{isAdmin&&<button onClick={()=>setShowInvite(true)} style={{background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"8px 18px",fontSize:13,cursor:"pointer",fontWeight:600}}>{t("invite_member")}</button>}</div>
        ):(
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{borderBottom:"1px solid var(--border)",background:"var(--bg-overlay)"}}>
              {[t("member_name"),t("email"),t("member_role"),t("status"),t("team_col_last_login","Last Login"),t("actions")].map(h=><th key={h} style={th}>{h}</th>)}
            </tr></thead>
            <tbody>
                ...
                <tr key={u.id} data-testid={`row-user-${u.id}`} style={{borderBottom:i<users.length-1?"1px solid var(--border)":"none",transition:"background 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.background="var(--bg-overlay)")} onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>
                  <td style={{padding:"12px 14px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div>
                        <div style={{fontWeight:600,fontSize:13,display:"flex",alignItems:"center",gap:6}}>{u.firstName?`${u.firstName} ${u.lastName||""}`.trim():u.email}{u.id===me?.id&&<span style={{fontSize:10,background:"rgba(99,102,241,0.12)",color:"#818cf8",padding:"1px 6px",borderRadius:4}}>{t("team_you_badge","You")}</span>}</div>
                  ...
                  <td style={{padding:"12px 14px"}}>
                    <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:6,background:u.isActive!==false?"rgba(34,197,94,0.12)":"rgba(239,68,68,0.12)",color:u.isActive!==false?"#4ade80":"#f87171"}}>
                      {u.isActive!==false?<><UserCheck size={10}/>{t("active")}</>:<><UserX size={10}/>{t("inactive")}</>}
                    </span>
                  </td>
                  <td style={{padding:"12px 14px",fontSize:12,color:"var(--text-muted)"}}>{u.lastLoginAt?<div style={{display:"flex",alignItems:"center",gap:4}}><Clock size={10}/>{new Date(u.lastLoginAt).toLocaleDateString()}</div>:t("team_never_logged_in","Never")}</td>
                  <td style={{padding:"12px 14px"}}>
                    {isAdmin&&u.id!==me?.id&&(
                      <div style={{display:"flex",gap:4}}>
                        <button data-testid={`button-edit-${u.id}`} onClick={()=>openEdit(u)} title={t("edit")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:5,borderRadius:6,display:"flex"}}><Edit2 size={13}/></button>
                        {me?.role==="super_admin"&&<button data-testid={`button-delete-${u.id}`} onClick={()=>setDel(u.id)} title={t("delete")} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:5,borderRadius:6,display:"flex"}}><Trash2 size={13}/></button>}
                      </div>
                    )}
                  </td>
                </tr>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:14,width:"100%",maxWidth:480}}>
            <div style={{padding:"18px 22px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontSize:16,fontWeight:700}}>{t("team_invite_modal_title","Invite Team Member")}</div>
              <button onClick={()=>{setShowInvite(false);setInviteResult(null);}} aria-label={t("close")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:4}}><X size={16}/></button>
            </div>
              ...
              {inviteResult?(
                <div>
                  <div style={{display:"flex",gap:10,alignItems:"flex-start",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10,padding:16,marginBottom:16}}>
                    ...
                    <div><div style={{fontWeight:700,marginBottom:4}}>{t("team_invite_sent_title","Invitation sent!")}</div><div style={{fontSize:13,color:"var(--text-secondary)"}}>{t("team_invite_sent_desc","Share these credentials with")} {inviteResult.email}</div></div>
                  </div>
                  <div style={{background:"var(--bg)",border:"1px solid var(--border)",borderRadius:8,padding:14,marginBottom:16}}>
                    <div style={{fontSize:11,color:"var(--text-muted)",marginBottom:6}}>{t("team_temp_password","Temporary Password")}</div>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
                      <code style={{fontSize:16,fontFamily:"monospace",fontWeight:700,letterSpacing:2}}>{inviteResult.tempPassword}</code>
                      <button onClick={()=>copyToClipboard(inviteResult.tempPassword)} style={{display:"flex",alignItems:"center",gap:4,background:"var(--bg-overlay)",border:"1px solid var(--border)",borderRadius:6,padding:"4px 8px",fontSize:11,cursor:"pointer",color:"var(--text-secondary)"}}><Copy size={11}/>{t("copy")}</button>
                    </div>
                    <div style={{fontSize:11,color:"#f59e0b",marginTop:8}}>{t("team_share_password_hint","Share this password securely — the user should change it on first login")}</div>
                  </div>
                  <button onClick={()=>{setShowInvite(false);setInviteResult(null);}} style={{width:"100%",background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"10px 0",fontSize:13,fontWeight:600,cursor:"pointer"}}>{t("close")}</button>
                </div>
              ):(
                <form onSubmit={submitInvite} style={{display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    {[{l:t("first_name"),k:"firstName"},{l:t("last_name"),k:"lastName"}].map(f=>(
                    ...
                    <div style={{gridColumn:"1/-1"}}><label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("email_address")} *</label><input data-testid="input-invite-email" type="email" required value={invite.email} onChange={e=>setInvite(p=>({...p,email:e.target.value}))} style={{width:"100%",padding:"9px 12px",background:"var(--bg)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",boxSizing:"border-box"}}/></div>
                    <div style={{gridColumn:"1/-1"}}><label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("member_role")}</label><select value={invite.role} onChange={e=>setInvite(p=>({...p,role:e.target.value}))} style={{width:"100%",padding:"9px 12px",background:"var(--bg)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",appearance:"none"}}>{ROLES.map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select></div>
                  </div>
                  <div style={{background:"var(--bg-overlay)",borderRadius:8,padding:"10px 12px",fontSize:12,color:"var(--text-muted)"}}>{t("team_temp_pwd_hint","A temporary password will be generated. Share it securely with the new member.")}</div>
                  <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                    <button type="button" onClick={()=>setShowInvite(false)} style={{background:"var(--bg-overlay)",border:"1px solid var(--border)",borderRadius:8,padding:"9px 18px",fontSize:13,cursor:"pointer"}}>{t("cancel")}</button>
                    <button data-testid="button-send-invite" type="submit" disabled={sendInvite.isPending} style={{background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{sendInvite.isPending?t("team_inviting","Inviting…"):t("send_invite")}</button>
                  </div>
                </form>
              )}
            </div>
      ...
      {editUser&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:14,width:"100%",maxWidth:400}}>
            <div style={{padding:"18px 22px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontSize:16,fontWeight:700}}>{t("team_edit_member_title","Edit Member")}</div>
              <button onClick={()=>setEditUser(null)} aria-label={t("close")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:4}}><X size={16}/></button>
            </div>
            <form onSubmit={submitEdit} style={{padding:"20px 22px",display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div><label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("member_role")}</label><select value={editForm.role} onChange={e=>setEditForm(p=>({...p,role:e.target.value}))} style={{width:"100%",padding:"9px 12px",background:"var(--bg)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",appearance:"none"}}>{ROLES.map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select></div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div><div style={{fontSize:13,fontWeight:600}}>{t("team_account_status","Account Status")}</div><div style={{fontSize:12,color:"var(--text-muted)"}}>{editForm.isActive?t("team_status_active_desc","Active and can log in"):t("team_status_blocked_desc","Blocked from logging in")}</div></div>
                <button type="button" onClick={()=>setEditForm(p=>({...p,isActive:!p.isActive}))} aria-label={t("team_toggle_status","Toggle account status")} style={{position:"relative",width:44,height:24,borderRadius:12,border:"none",background:editForm.isActive?"var(--accent)":"var(--border)",cursor:"pointer",transition:"background 0.2s",flexShrink:0}}>
              ...
              <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                <button type="button" onClick={()=>setEditUser(null)} style={{background:"var(--bg-overlay)",border:"1px solid var(--border)",borderRadius:8,padding:"9px 18px",fontSize:13,cursor:"pointer"}}>{t("cancel")}</button>
                <button type="submit" disabled={updateUser.isPending} style={{background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{updateUser.isPending?t("team_saving","Saving…"):t("save_changes")}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {del&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:28,maxWidth:380,width:"90%"}}>
            <div style={{display:"flex",gap:12,marginBottom:16}}><AlertCircle size={20} style={{color:"#ef4444",flexShrink:0,marginTop:2}}/><div><div style={{fontWeight:700,marginBottom:4}}>{t("team_remove_confirm_title","Remove Member?")}</div><div style={{fontSize:13,color:"var(--text-muted)"}}>{t("team_remove_confirm_desc","This will permanently remove this user from your workspace.")}</div></div></div>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              ...
              <button onClick={()=>deleteUser.mutate(del!)} disabled={deleteUser.isPending} style={{background:"#ef4444",color:"#fff",border:"none",borderRadius:8,padding:"8px 16px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{deleteUser.isPending?t("team_removing","Removing…"):t("remove")}</button>
            </div>
          </div>
        </div>
      )}
