...
function ScoreBadge({ score }: { score: number }) {
  ...
  const label = score >= 60 ? t("etrack_score_hot", "Hot") : score >= 30 ? t("etrack_score_warm", "Warm") : t("etrack_score_cold", "Cold");
  return (
...
function ComposeModal({ onClose }: { onClose: () => void }) {
  ...
  const send = async () => {
    if (!form.toEmail || !form.subject || !form.body) { setError(t("etrack_fields_required", "All fields are required")); return; }
    ...
    try {
      ...
      setSuccess(t("etrack_sent_success", `Email sent to ${form.toEmail} with tracking enabled`).replace("{email}", form.toEmail));
      qc.invalidateQueries({ queryKey: ["/api/email/analytics"] });
      qc.invalidateQueries({ queryKey: ["/api/email/sends"] });
    } catch (e: any) {
      setError(e.message || t("etrack_send_failed", "Failed to send email"));
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "var(--bg-card)", borderRadius: 14, width: "100%", maxWidth: 560, padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 16 }}>{t("etrack_compose_title", "Compose Tracked Email")}</div>
        ...
        {success ? (
          <div style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 10, padding: 16, color: "#10b981", fontWeight: 600, textAlign: "center" }}>
            {success}
            <div style={{ fontSize: 12, marginTop: 6, opacity: 0.8 }}>{t("etrack_tracking_note", "Opens, clicks, and forwards will appear in your dashboard automatically.")}</div>
            <button onClick={onClose} style={{ marginTop: 14, background: "#10b981", color: "#fff", border: "none", borderRadius: 8, padding: "8px 20px", cursor: "pointer", fontWeight: 600 }}>{t("etrack_done", "Done")}</button>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, display: "block", marginBottom: 4 }}>{t("etrack_to_email", "To Email")} *</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, display: "block", marginBottom: 4 }}>{t("etrack_to_name", "To Name")}</label>
            ...
            <div>
              <label style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, display: "block", marginBottom: 4 }}>{t("etrack_subject", "Subject")} *</label>
              <input data-testid="input-subject" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                placeholder={t("etrack_subject_ph", "Email subject")} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text)", boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, display: "block", marginBottom: 4 }}>{t("etrack_message", "Message")} *</label>
              <textarea data-testid="input-body" value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))}
                placeholder={t("etrack_message_ph", "Write your message here...")} rows={8}
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button onClick={onClose} style={{ padding: "9px 18px", borderRadius: 8, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--text)" }}>{t("cancel", "Cancel")}</button>
              <button data-testid="button-send-email" onClick={send} disabled={sending}
                ...
                <Send size={14} /> {sending ? t("etrack_sending", "Sending…") : t("etrack_send_track", "Send & Track")}
              </button>
            </div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", textAlign: "center", paddingTop: 4 }}>
              {t("etrack_pixel_note", "A tracking pixel and click-tracking links will be automatically embedded in your email.")}
            </div>
          </div>
        )}
      </div>
    ...
    <div style={{ border: "1px solid var(--border)", borderRadius: 10, marginBottom: 8, overflow: "hidden" }}>
      <div onClick={onExpand} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", background: "var(--bg-card)" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{t("etrack_to_label", "To")}: {send.contact_name || send.to_email} · {new Date(send.sent_at).toLocaleDateString()}</div>
        </div>
      ...
      {expanded && (
        <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", background: "var(--bg)", fontSize: 12 }}>
          {opens === 0 && clicks === 0 && forwards === 0
            ? <span style={{ color: "var(--text-muted)" }}>{t("etrack_no_engagement", "No engagement yet — email may not have been opened.")}</span>
            : <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                {opens > 0 && <span style={{ color: "#3b82f6" }}><Eye size={11} style={{ display: "inline", marginRight: 4 }} />{opens} {t("etrack_open_lc", "open")}{opens !== 1 ? "s" : ""}</span>}
                {clicks > 0 && <span style={{ color: "#10b981" }}><MousePointer size={11} style={{ display: "inline", marginRight: 4 }} />{clicks} {t("etrack_click_lc", "click")}{clicks !== 1 ? "s" : ""}</span>}
                {forwards > 0 && <span style={{ color: "#f59e0b" }}><Share2 size={11} style={{ display: "inline", marginRight: 4 }} />{forwards} {t("etrack_forward_lc", "forward")}{forwards !== 1 ? "s" : ""} {t("etrack_detected", "detected")}</span>}
              </div>
          }
          {send.first_opened_at && <div style={{ marginTop: 6, color: "var(--text-muted)" }}>{t("etrack_first_opened", "First opened")}: {new Date(send.first_opened_at).toLocaleString()}</div>}
        </div>
      )}
    </div>
  );
...
export default function EmailTrackingPage() {
  ...
  const tabs = [
    { id: "overview", label: t("etrack_tab_overview", "Overview") },
    { id: "sends", label: `${t("etrack_tab_sent", "Sent")} (${sends.length})` },
    { id: "hot_leads", label: `${t("etrack_tab_hot_leads", "Hot Leads")} (${hotLeads.length})` },
    { id: "activity", label: t("etrack_tab_activity", "Live Activity") },
  ];

  return (
    <Layout title={t("nav_email_tracking", "Email Tracking")} subtitle={t("etrack_subtitle", "Track opens, clicks, and forwards in real time")}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div style={{ display: "flex", gap: 8 }}>
          {tabs.map(tabItem => (
            <button key={tabItem.id} onClick={() => setTab(tabItem.id as any)}
              style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid var(--border)", background: tab === tabItem.id ? "var(--primary)" : "var(--bg-card)", color: tab === tabItem.id ? "#fff" : "var(--text)", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
              {tabItem.label}
            </button>
          ))}
        </div>
        <button data-testid="button-compose" onClick={() => setCompose(true)}
          ...
          <Send size={14} /> {t("etrack_compose_btn", "Compose & Track")}
        </button>
      </div>
      ...
      {tab === "overview" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 28 }}>
            <StatCard icon={Mail} label={t("etrack_stat_sent", "Total Sent")} value={totals.total_sent} color="#6366f1" />
            <StatCard icon={Eye} label={t("etrack_stat_opens", "Opens")} value={totals.total_opened} sub={totals.open_rate ? `${totals.open_rate}% ${t("etrack_open_rate", "open rate")}` : undefined} color="#3b82f6" />
            <StatCard icon={MousePointer} label={t("etrack_stat_clicks", "Clicks")} value={totals.total_clicked} sub={totals.click_rate ? `${totals.click_rate}% ${t("etrack_click_rate", "click rate")}` : undefined} color="#10b981" />
            <StatCard icon={Share2} label={t("etrack_stat_forwards", "Forwards Detected")} value={totals.total_forwarded} color="#f59e0b" />
            <StatCard icon={Zap} label={t("etrack_stat_hot", "Hot Leads")} value={hotLeads.filter((l: any) => l.engagement_score >= 60).length} sub={t("etrack_score_60plus", "Score 60+")} color="#ef4444" />
          </div>

          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, padding: 20, marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{t("etrack_scores_title", "How scores are calculated")}</div>
            <div style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 14 }}>{t("etrack_scores_desc", "Every engagement action updates the contact's score in real time.")}</div>
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[
                [t("etrack_event_opened", "Email opened"), t("etrack_pts_open", "+5 pts"), "#3b82f6"],
                [t("etrack_event_clicked", "Link clicked"), t("etrack_pts_click", "+15 pts"), "#10b981"],
                [t("etrack_event_forwarded", "Forward detected"), t("etrack_pts_forward", "+25 pts"), "#f59e0b"],
              ].map(([label, pts, color]) => (
                <div key={String(label)} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: `${color}18`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <TrendingUp size={14} style={{ color: String(color) }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{label}</div>
                    <div style={{ fontWeight: 700, color: String(color) }}>{pts}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {hotLeads.length > 0 && (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, padding: 20 }}>
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>{t("etrack_top_engaged", "Top Engaged Contacts")}</div>
                ...
                {hotLeads.slice(0, 5).map((lead: any) => (
                  <div key={lead.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "var(--bg)", borderRadius: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 12, color: "var(--text-muted)" }}>
                      <span title={t("etrack_stat_opens", "Opens")}><Eye size={11} /> {lead.email_opens || 0}</span>
                      <span title={t("etrack_stat_clicks", "Clicks")}><MousePointer size={11} /> {lead.email_clicks || 0}</span>
                      <span title={t("etrack_stat_forwards", "Forwards Detected")}><Share2 size={11} /> {lead.email_forwards || 0}</span>
                    </div>
          ...
          {!isLoading && sends.length === 0 && (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}>
              ...
              <div style={{ fontWeight: 600, marginBottom: 6 }}>{t("etrack_no_emails", "No tracked emails yet")}</div>
              <div style={{ fontSize: 13, marginBottom: 18 }}>{t("etrack_no_emails_desc", "Send your first tracked email to start seeing opens, clicks, and forwards.")}</div>
              <button onClick={() => setCompose(true)} style={{ padding: "10px 22px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#3b82f6,#6366f1)", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
                {t("etrack_send_first", "Send your first tracked email")}
              </button>
            </div>
          )}
        </div>
      ...
      {tab === "sends" && (
        <div>
          {sends.length === 0
            ? <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}><Mail size={36} style={{ opacity: 0.3, marginBottom: 10 }} /><div>{t("etrack_no_sent", "No emails sent yet")}</div></div>
            : sends.map((s: any) => (
      ...
      {tab === "hot_leads" && (
        <div>
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>{t("etrack_hot_leads_desc", "Contacts ranked by engagement score — the higher the score, the warmer the lead.")}</div>
          {hotLeads.length === 0
            ? <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}><Users size={36} style={{ opacity: 0.3, marginBottom: 10 }} /><div>{t("etrack_no_engagement_data", "No engagement data yet")}</div></div>
            : hotLeads.map((lead: any) => (
              <div key={lead.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 10, marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 14, fontSize: 13, color: "var(--text-secondary)", flexShrink: 0 }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4 }}><Eye size={13} style={{ color: "#3b82f6" }} />{lead.email_opens || 0} {t("etrack_opens_lc", "opens")}</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 4 }}><MousePointer size={13} style={{ color: "#10b981" }} />{lead.email_clicks || 0} {t("etrack_clicks_lc", "clicks")}</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 4 }}><Share2 size={13} style={{ color: "#f59e0b" }} />{lead.email_forwards || 0} {t("etrack_forwards_lc", "forwards")}</span>
                </div>
                <ScoreBadge score={lead.engagement_score || 0} />
      ...
      {tab === "activity" && (
        <div>
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>{t("etrack_activity_desc", "Real-time feed of every email engagement event across all your tracked emails.")}</div>
          {recentEvents.length === 0
            ? <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}><Clock size={36} style={{ opacity: 0.3, marginBottom: 10 }} /><div>{t("etrack_no_activity", "No activity yet")}</div></div>
                ...
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 0", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13 }}>
                      ...
                      {ev.is_forward && <span style={{ marginLeft: 6, background: "#f59e0b18", color: "#f59e0b", padding: "1px 7px", borderRadius: 10, fontSize: 10, fontWeight: 700 }}>{t("etrack_forward_badge", "FORWARD")}</span>}
                    </div>
