...
function EmailComposerTab() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <SectionCard title={t("aitools_email_context", "Email Context")} icon={Mail} color="#3b82f6">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {[
            { key: "contactName", label: t("contact_name", "Contact Name"), placeholder: "Jane Smith" },
            { key: "company", label: t("company", "Company"), placeholder: "Acme Corp" },
            { key: "dealName", label: t("aitools_deal_name_optional", "Deal Name (optional)"), placeholder: "Enterprise Plan" },
          ...
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_purpose", "Purpose")}</label>
          ...
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_tone", "Tone")}</label>
        ...
        <div style={{ marginTop: 12 }}>
          <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_additional_context", "Additional Context (optional)")}</label>
          <textarea
            value={form.additionalContext}
            onChange={e => set("additionalContext", e.target.value)}
            placeholder={t("aitools_additional_context_ph", "E.g. We met at SaaStr, they mentioned a Q2 budget…")}
            rows={3}
        ...
        >
          {mutation.isPending ? <><RefreshCw size={13} style={{ animation: "spin 1s linear infinite" }} /> {t("aitools_writing", "Writing…")}</> : <><Zap size={13} /> {t("aitools_generate_email", "Generate Email")}</>}
        </button>
        {mutation.error && <UpgradeBanner error={mutation.error} />}
      </SectionCard>

      {result && (
        <SectionCard title={t("aitools_generated_email", "Generated Email")} icon={Mail} color="#10b981">
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{t("aitools_subject_line", "Subject line")}</span>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{t("aitools_email_body", "Email body")}</span>
          ...
          {result.followUpTips?.length > 0 && (
            <div style={{ marginTop: 14, padding: "10px 14px", background: "#f59e0b10", borderRadius: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#f59e0b", marginBottom: 6 }}>{t("aitools_send_tips", "Send Tips")}</div>
...
function MeetingSummarizerTab() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <SectionCard title={t("aitools_meeting_transcript", "Meeting Transcript")} icon={Mic} color="#8b5cf6">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_contact_name_optional", "Contact Name (optional)")}</label>
            <input value={contactName} onChange={e => setContactName(e.target.value)} placeholder="Jane Smith"
              style={{ width: "100%", padding: "9px 12px", background: "var(--bg-overlay)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-primary)", fontSize: 13, boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_deal_name_optional", "Deal Name (optional)")}</label>
            <input value={dealName} onChange={e => setDealName(e.target.value)} placeholder="Enterprise Plan"
              style={{ width: "100%", padding: "9px 12px", background: "var(--bg-overlay)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-primary)", fontSize: 13, boxSizing: "border-box" }} />
          </div>
        </div>
        <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>{t("aitools_paste_transcript", "Paste your meeting transcript")}</label>
        <textarea
          value={transcript}
        ...
        >
          {mutation.isPending ? <><RefreshCw size={13} style={{ animation: "spin 1s linear infinite" }} /> {t("aitools_analyzing", "Analyzing…")}</> : <><Brain size={13} /> {t("aitools_analyze_meeting", "Analyze Meeting")}</>}
        </button>
        {mutation.error && <UpgradeBanner error={mutation.error} />}
      </SectionCard>
      ...
      {result && (
        <>
          <SectionCard title={t("aitools_summary", "Summary")} icon={Brain} color="#8b5cf6">
            <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              {result.suggestedStageUpdate && (
                <span style={{ padding: "4px 10px", background: "#3b82f620", color: "#3b82f6", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                  {t("aitools_suggested_move", "Suggested: Move to")} {result.suggestedStageUpdate}
                </span>
              )}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <SectionCard title={t("aitools_action_items", "Action Items")} icon={Check} color="#10b981">
              {result.actionItems?.length === 0 ? (
                <div style={{ fontSize: 13, color: "var(--text-muted)" }}>{t("aitools_no_action_items", "No action items identified")}</div>
              ) : (
                result.actionItems?.map((a: any, i: number) => (
                  <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10, padding: "8px 10px", background: "var(--bg-overlay)", borderRadius: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{a.owner} · {a.dueDate || t("aitools_no_deadline", "No deadline")}</div>
                    </div>
                  </div>
                ))
              )}
            ...
            <SectionCard title={t("aitools_key_points_objections", "Key Points & Objections")} icon={Lightbulb} color="#f59e0b">
