...
export default function InvoicesPage() {
  ...
  return (
    <Layout title={t("invoices_title")} subtitle={`${filtered.length} ${t("nav_invoices").toLowerCase()}`} actions={
      <button data-testid="button-add-invoice" onClick={()=>open()} style={{display:"flex",alignItems:"center",gap:6,background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"7px 14px",fontSize:13,fontWeight:600,cursor:"pointer"}}><Plus size={14}/>{t("create_invoice")}</button>
    }>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        {[
          {l:t("invoices_stat_total","Total Invoices"),v:stats.total,I:FileText,c:"#6366f1"},
          {l:t("invoices_stat_collected","Revenue Collected"),v:fmtCurrency(stats.paid),I:CheckCircle,c:"#22c55e"},
          {l:t("invoices_stat_outstanding","Outstanding"),v:fmtCurrency(stats.outstanding),I:Clock,c:"#f59e0b"},
          {l:t("invoice_overdue"),v:stats.overdue,I:AlertTriangle,c:"#ef4444"}
        ].map(s=>(
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200,position:"relative"}}><Search size={14} style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"var(--text-muted)"}}/><input data-testid="input-search-invoices" value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("invoices_search_ph","Search invoices…")} style={{width:"100%",paddingLeft:32,paddingRight:12,paddingTop:8,paddingBottom:8,background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",boxSizing:"border-box"}}/></div>
        <div style={{position:"relative"}}>
          <select data-testid="select-status" value={sf} onChange={e=>setSf(e.target.value)} style={{paddingLeft:12,paddingRight:28,paddingTop:8,paddingBottom:8,background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",appearance:"none",cursor:"pointer"}}>
            <option value="all">{t("invoices_all_statuses","All Statuses")}</option>
      ...
      <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:10,overflow:"hidden"}}>
        {isLoading?<div style={{padding:48,textAlign:"center",color:"var(--text-muted)"}}>{t("loading")}</div>:filtered.length===0?(
          <div style={{padding:64,textAlign:"center"}}><FileText size={36} style={{color:"var(--text-muted)",marginBottom:12,opacity:0.4}}/><div style={{fontSize:15,fontWeight:600,marginBottom:6}}>{t("invoices_empty_title","No invoices found")}</div><div style={{fontSize:13,color:"var(--text-muted)",marginBottom:16}}>{search?t("invoices_empty_search","Try a different search"):t("no_invoices")}</div>{!search&&<button onClick={()=>open()} style={{background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"8px 18px",fontSize:13,cursor:"pointer",fontWeight:600}}>{t("create_invoice")}</button>}</div>
        ):(
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{borderBottom:"1px solid var(--border)",background:"var(--bg-overlay)"}}>
              {[t("invoice_number"),t("status"),t("invoices_col_items","Items"),t("invoices_col_subtotal","Subtotal"),t("tax"),t("total"),t("invoice_due"),t("actions")].map(h=><th key={h} style={th}>{h}</th>)}
            </tr></thead>
            <tbody>
              {filtered.map((inv:any,i:number)=>(
                <tr key={inv.id} data-testid={`row-invoice-${inv.id}`} style={{borderBottom:i<filtered.length-1?"1px solid var(--border)":"none",transition:"background 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.background="var(--bg-overlay)")} onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>
                  ...
                  <td style={{padding:"12px 14px",fontSize:13,color:"var(--text-secondary)"}}>{(inv.items||[]).length} {t("items")}</td>
                  ...
                  <td style={{padding:"12px 14px"}}>
                    <div style={{display:"flex",gap:4}}>
                      <button data-testid={`button-edit-${inv.id}`} onClick={()=>open(inv)} title={t("edit")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:5,borderRadius:6,display:"flex"}}><Edit2 size={13}/></button>
                      <button data-testid={`button-delete-${inv.id}`} onClick={()=>setDel(inv.id)} title={t("delete")} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:5,borderRadius:6,display:"flex"}}><Trash2 size={13}/></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {del&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:28,maxWidth:380,width:"90%"}}>
            <div style={{display:"flex",gap:12,marginBottom:16}}><AlertCircle size={20} style={{color:"#ef4444",flexShrink:0,marginTop:2}}/><div><div style={{fontWeight:700,marginBottom:4}}>{t("invoices_delete_confirm_title","Delete Invoice?")}</div><div style={{fontSize:13,color:"var(--text-muted)"}}>{t("cannot_be_undone")}</div></div></div>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              ...
              <button onClick={()=>rm.mutate(del!)} disabled={rm.isPending} style={{background:"#ef4444",color:"#fff",border:"none",borderRadius:8,padding:"8px 16px",fontSize:13,fontWeight:600,cursor:"pointer"}}>{rm.isPending?t("invoices_deleting","Deleting…"):t("delete")}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:14,width:"100%",maxWidth:700,maxHeight:"92vh",overflowY:"auto"}}>
            <div style={{padding:"18px 24px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:16,fontWeight:700}}>{editing?t("invoices_edit_title","Edit Invoice"):t("create_invoice")}</div><button onClick={close} aria-label={t("close")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text-muted)",padding:4}}><X size={16}/></button></div>
            <form onSubmit={submit} style={{padding:"20px 24px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:20}}>
                <div>
                  <label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("invoice_number")} *</label>
                ...
                <div>
                  <label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("invoice_due")}</label>
              ...
              <div style={{marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <div style={{fontSize:13,fontWeight:700}}>{t("invoices_line_items","Line Items")}</div>
                  <button type="button" onClick={addItem} style={{display:"flex",alignItems:"center",gap:4,background:"var(--bg-overlay)",border:"1px solid var(--border)",borderRadius:7,padding:"5px 10px",fontSize:12,fontWeight:600,cursor:"pointer",color:"var(--text-primary)"}}><Plus size={12}/>{t("invoices_add_item","Add Item")}</button>
                </div>
                {form.items.length===0?<div style={{padding:"20px",textAlign:"center",border:"2px dashed var(--border)",borderRadius:8,fontSize:13,color:"var(--text-muted)"}}>{t("invoices_no_items","No items yet — click \"Add Item\" to start")}</div>:(
                  <div style={{border:"1px solid var(--border)",borderRadius:8,overflow:"hidden"}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr style={{background:"var(--bg-overlay)"}}>
                        {[t("description"),t("invoices_col_qty","Qty"),t("invoices_col_unit_price","Unit Price"),t("total"),""].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:700,textTransform:"uppercase",color:"var(--text-muted)"}}>{h}</th>)}
                      </tr></thead>
                      <tbody>
                        {form.items.map((it:LineItem,idx:number)=>(
                          <tr key={idx} style={{borderTop:"1px solid var(--border)"}}>
                            <td style={{padding:"8px 12px"}}><input value={it.description} onChange={e=>updateItem(idx,"description",e.target.value)} placeholder={t("invoices_item_desc_ph","Item description")} style={{width:"100%",padding:"6px 8px",background:"var(--bg)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text-primary)",fontSize:12,outline:"none",boxSizing:"border-box"}}/></td>
                            ...
                            <td style={{padding:"8px 12px"}}><button type="button" onClick={()=>removeItem(idx)} aria-label={t("remove")} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",display:"flex",padding:4}}><X size={12}/></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div style={{display:"flex",justifyContent:"flex-end",marginBottom:20}}>
                <div style={{background:"var(--bg-overlay)",border:"1px solid var(--border)",borderRadius:10,padding:"16px 20px",minWidth:280}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:13,color:"var(--text-secondary)"}}><span>{t("invoices_col_subtotal","Subtotal")}</span><span>${subtotal.toFixed(2)}</span></div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,fontSize:13,color:"var(--text-secondary)",gap:8}}>
                    <span style={{whiteSpace:"nowrap"}}>{t("invoices_tax_rate_label","Tax Rate (%)")}</span>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      ...
                      <span style={{fontSize:12,color:"var(--text-muted)"}}>{taxRatePct>0?`$${tax.toFixed(2)}`:t("invoices_no_tax","No tax")}</span>
                    </div>
                  </div>
                  <div style={{borderTop:"1px solid var(--border)",paddingTop:8,display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:16}}><span>{t("total")}</span><span>${total.toFixed(2)}</span></div>
                </div>
              </div>

              <div>
                <label style={{display:"block",fontSize:12,fontWeight:600,marginBottom:5,color:"var(--text-secondary)"}}>{t("notes")}</label>
                <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={2} placeholder={t("invoices_notes_ph","Payment terms, notes for client…")} style={{width:"100%",padding:"9px 12px",background:"var(--bg)",border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",fontSize:13,outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
              </div>
              ...
              <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:20}}>
                ...
                <button data-testid="button-submit-invoice" type="submit" disabled={create.isPending||upd.isPending} style={{background:"var(--accent)",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:600,cursor:"pointer"}}>
                  {create.isPending||upd.isPending ? t("invoices_saving","Saving…") : editing ? t("save_changes") : t("create_invoice")}
                </button>
              </div>
            </form>
          </div>
