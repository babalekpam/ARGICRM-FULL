...
### Recent Fixes
- **Super Admin Dashboard Device Discrepancy Fix (2025-11-07)**: Enhanced Super Admin Dashboard with helpful context alerts explaining why user counts differ between devices. Added blue info alert showing total users (31) with clear explanation: platform owners (abel@argilette.com, admin@default.com) see all users across all tenants, while regular users cannot access the dashboard (403 error). Includes tip that different numbers on different devices means different login accounts. Added error handling for registration data query with retry disabled for 403 errors.
- **Navigation Link Bug Fix (2025-11-07)**: Fixed broken navigation across 18 pages (Privacy, Pricing, About, Features, Terms, Blog, Enterprise CRM, Small Business CRM, AI-Powered CRM, Help Center, Overview, Solutions, What is CRM, Dashboard, Forgot Password, Reset Password, and backup landing pages). Changed 90 Link instances from incorrect React Router syntax `to=` to correct wouter syntax `href=`. Also fixed 9 files importing Link from "react-router-dom" instead of "wouter". All navigation links (Features, Pricing, About, Home) now work correctly across the entire platform.
