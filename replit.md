...
Security Hardening: All emergency login bypass pages deleted (autologin.html, debug-login.html, direct-login.html, emergency-login.html, quick-login.html, etc.). Platform uses secure bcrypt password hashing. Database contains ONLY 1 unique platform owner account. **Account Protection**: Platform owner account (abel@argilette.com) is the unique super admin with full platform privileges - fully protected from deactivation and role changes via triple-layer security: (1) Backend API validation with strict type checking to prevent type coercion bypasses, (2) Database layer enforcement blocking all unauthorized modifications, (3) Frontend UI controls disabled with clear visual indicators. Protection uses email-based immutable identifier (abel@argilette.com) and strict !== undefined checks to prevent falsy value exploits (0, null, "", "false"). Quick Login button removed from landing page for enhanced security.
...
- **AI Funnel Builder (November 2025)**: Comprehensive AI-powered sales funnel generation system that transforms user offers into complete marketing funnels with one-click generation. Features include: 14-table database architecture (funnel_projects, funnel_versions, funnel_steps, landing_pages with hero_image_url, funnel_ads, funnel_emails, automation_workflows, ai_generations, plus 6 analytics/publication tables); AI-powered generation of landing pages with profession-specific stock imagery (healthcare/medical, SaaS, ecommerce, consulting, coaching), hero content/benefits/testimonials/FAQs, multi-platform ad copy (Facebook/Google/LinkedIn), email sequences with delays, and automation workflows; comprehensive funnel management with versioning, cloning, templates, and publishing; visual landing page editor with preview/edit modes, inline field editing, and "View Landing Page" external preview button with security (noopener,noreferrer); tabbed content display for landing pages/ads/emails/workflows; strict tenant isolation with DatabaseStorage validation; real-time generation feedback via Argilette AI (Replit AI Integrations); backend data transformation layer (snake_case → camelCase) for frontend compatibility; TanStack Query integration with explicit apiRequest calls for authenticated routes; complete analytics tracking (page views, conversions, form submissions, email events, ad performance); industry-specific visual asset mapping with 6 curated stock images stored in attached_assets/stock_images/; and support for A/B testing different funnel versions. Navigation accessible via Marketing > AI Funnel Builder. Replaces manual multi-tool workflows (ClickFunnels + AdEspresso + Mailchimp + Unsplash) with unified one-click funnel creation.
