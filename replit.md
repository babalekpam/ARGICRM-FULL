...
AI Integration Migration: Migrated from Anthropic Claude to Replit AI Integrations with COMPLETE WHITE-LABEL branding as "Argilette AI". All AI services (SEO insights, campaign generation, sentiment analysis, content generation, Cloe AI Agent) now use cost-effective Replit AI Integrations with OpenAI GPT-5 fully white-labeled. Internal provider identifier changed from "openai" to "argilette" to completely hide OpenAI references from end users in API responses, logs, and UI. API responses show `"provider":"argilette"` with zero OpenAI exposure. Settings page displays "Argilette AI Key" instead of "OpenAI API Key". Server logs show "Argilette AI" branding throughout. Charges billed to Replit credits. Multi-provider AI failover system provides redundancy (Argilette AI primary, Anthropic fallback, You.com, Google, Qwen). Fixed Cloe AI Agent to use Replit AI Integrations instead of legacy OpenAI direct integration - updated all model references from gpt-4o to gpt-5, changed environment variable checks from OPENAI_API_KEY to AI_INTEGRATIONS_OPENAI_BASE_URL and AI_INTEGRATIONS_OPENAI_API_KEY. Key files: server/ai.ts, server/argilette/seo-ai.ts, server/services/ai-failover-service.ts, server/services/cloe-ai-agent.ts, client/src/pages/settings.tsx.
...
SEO Audit Enhancement: Added complete website audit workflow with dialog-based URL input interface - Users can now add website URLs directly through a clean dialog form, run comprehensive SEO audits analyzing performance, accessibility, best practices, and Core Web Vitals, and view detailed audit results with issue prioritization. Features include website URL input with validation, project selection for multi-site management, real-time audit progress indicators, integration with backend `/api/seo/technical-audit/start` endpoint, and comprehensive results display showing critical issues, warnings, and improvement suggestions. Resolves UX gap where users had no way to initiate audits. Key file: client/src/pages/seo-audit.tsx.
