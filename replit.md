...
- **AI Funnel Builder (November 2025)**: Comprehensive AI-powered sales funnel generation system that transforms user offers into complete marketing funnels with one-click generation. Features include: 14-table database architecture (funnel_projects, funnel_versions, funnel_steps, landing_pages, funnel_ads, funnel_emails, automation_workflows, ai_generations, plus 6 analytics/publication tables); AI-powered generation of landing pages with hero content/benefits/testimonials/FAQs, multi-platform ad copy (Facebook/Google/LinkedIn), email sequences with delays, and automation workflows; comprehensive funnel management with versioning, cloning, templates, and publishing; visual landing page editor with preview/edit modes and inline field editing; tabbed content display for landing pages/ads/emails/workflows; strict tenant isolation with DatabaseStorage validation; real-time generation feedback via Argilette AI (Replit AI Integrations); backend data transformation layer (snake_case → camelCase) for frontend compatibility; TanStack Query integration with explicit apiRequest calls for authenticated routes; complete analytics tracking (page views, conversions, form submissions, email events, ad performance); and support for A/B testing different funnel versions. Navigation accessible via Marketing > AI Funnel Builder. Replaces manual multi-tool workflows (ClickFunnels + AdEspresso + Mailchimp) with unified one-click funnel creation.
