  ...
  try {
    ...
    if (dealCount < 3) {
      await db.insert(deals).values([
        { tenantId: TENANT_ID, title: "TechSolutions Dakar — CRM Enterprise", value: "24000", stage: "closed_won", probability: 100, expectedCloseDate: new Date("2026-03-15"), contactId: demoContacts[0]?.id ?? null, notes: "12-month enterprise contract signed" },
        { tenantId: TENANT_ID, title: "FinTech Lagos — Growth Plan", value: "8400", stage: "proposal", probability: 65, expectedCloseDate: new Date("2026-04-30"), contactId: demoContacts[1]?.id ?? null, notes: "Proposal for 24-month Growth subscription" },
        { tenantId: TENANT_ID, title: "GrowthCo USA — Agency License", value: "47880", stage: "negotiation", probability: 80, expectedCloseDate: new Date("2026-04-15"), contactId: demoContacts[2]?.id ?? null, notes: "Agency Starter 12 workspaces — negotiating pricing" },
      ]);
    }
